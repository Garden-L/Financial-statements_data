...
class jamu:
    ...
    def modify_first(self):
        ...
        modify =pd.read_excel('계정과목3.xlsx')
        modify.set_index('항목명', inplace=True)
        ...
        for df in self.dfCopyList:
            ...
            # entity=df.loc[df['항목코드'].str.contains(r'^e'),'항목코드'].str.slice(0,14)
            df.loc[df['항목코드'].str.contains(r'^e'),'항목코드'] = float('NaN')
            #entity
            # tempIndex = df.loc[df['항목코드'].str.contains(r'^e'),:]

            df.set_index('항목명', inplace=True)
            df.update(modify, overwrite=False)
            df.reset_index(inplace=True)
            df.set_index('항목코드', inplace=True)
            df.update(ifrsmodify)
            df.reset_index(inplace=True)
